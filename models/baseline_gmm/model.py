...
class BaselineGMM(nn.Module):
    """
    Baseline GMM model for clustering in pixel space.
    ...
    def __init__(self, input_dim=784, num_clusters=10):
        super(BaselineGMM, self).__init__()
        self.input_dim = input_dim
        self.num_clusters = num_clusters
        self.scaler = StandardScaler()
        self.gmm = None

        # Calculate image size from input_dim (assumes square grayscale images)
        self.image_size = int(np.sqrt(input_dim))

        # Dummy parameter to make optimizer happy (will not be trained)
    ...
    def fit_gmm_and_evaluate(self, z_data, labels, covariance_type='diag', n_init=10):
        """Fit GMM on normalized pixel space."""
        ...
        gmm_labels = self.gmm.fit_predict(z_data)
        return {'gmm': self.gmm, 'gmm_labels': gmm_labels}


def get_model(config):
    """Factory function to create BaselineGMM model instance."""
    dataset_name = config['dataset']['name']
    input_dim = DATASET_TO_INPUT_DIM.get(dataset_name)
    if input_dim is None:
        raise ValueError(f"Baseline GMM not configured for dataset: {dataset_name}")
